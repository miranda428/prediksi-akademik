...
# ============================================================
    ...
    hero("💬 Konsultasi", "Media komunikasi pendukung antara siswa/orang tua dengan guru atau operator.", "🤝 Fitur komunikasi sistem")
    c = conn()

    # Semua akun dapat berkomunikasi dengan akun yang relevan.
    if role in ["Siswa", "Orang Tua/Wali"]:
        recipient_roles = ("Guru/Wali Kelas", "Operator", "Kepala Sekolah")
    else:
        recipient_roles = ("Siswa", "Orang Tua/Wali")

    placeholders = ",".join(["?"] * len(recipient_roles))
    recipients = c.execute(
        f"SELECT username,name,role FROM users WHERE role IN ({placeholders}) AND username<>? ORDER BY role,name",
        (*recipient_roles, u["username"])
    ).fetchall()
    msgs = c.execute(
        "SELECT sender,receiver,message,created_at FROM consultations "
    ...
    if choices:
        st.markdown('<div class="section-title">✉️ Kirim Pesan</div>', unsafe_allow_html=True)
        with st.form("consult_form"):
            to_label = st.selectbox("Kirim kepada", list(choices.keys()))
            message = st.text_area(
                "Pesan",
                placeholder="Tuliskan pertanyaan, informasi, atau tanggapan..."
            )
        ...
        if send:
            if not message.strip():
                ...
                c = conn()
                c.execute(
                    "INSERT INTO consultations(sender,receiver,message,created_at) VALUES(?,?,?,?)",
                    (
                        u["username"],
                        choices[to_label],
                        message.strip(),
        ...
        st.info("Belum ada akun penerima yang tersedia untuk konsultasi.")

    st.markdown('<div class="section-title">💬 Riwayat Percakapan</div>', unsafe_allow_html=True)
    if msgs:
        mdf = nice_columns(pd.DataFrame([dict(x) for x in msgs]))
        if "Tanggal" in mdf.columns:
            mdf["Tanggal"] = pd.to_datetime(mdf["Tanggal"]).dt.strftime("%d-%m-%Y %H:%M")
        st.dataframe(mdf, use_container_width=True, hide_index=True)

        # Balasan cepat untuk pesan yang masuk ke akun saat ini.
        incoming = [dict(x) for x in msgs if x["receiver"] == u["username"]]
        if incoming:
            st.markdown('<div class="section-title">↩️ Balas Pesan</div>', unsafe_allow_html=True)
            for i, item in enumerate(incoming):
                sender_name = item["sender"]
                st.markdown(
                    f'<div class="panel"><b>Dari:</b> {sender_name}<br>'
                    f'<b>Pesan:</b> {item["message"]}<br>'
                    f'<span class="small-muted">{item["created_at"]}</span></div>',
                    unsafe_allow_html=True
                )
                with st.form(f"reply_form_{i}"):
                    reply = st.text_area("Balasan", key=f"reply_text_{i}", placeholder="Tuliskan balasan...")
                    reply_btn = st.form_submit_button("📨 Kirim Balasan", use_container_width=True)
                if reply_btn:
                    if not reply.strip():
        ...
        st.info("Belum ada pesan. Silakan kirim pesan pertama melalui formulir di atas.")
