...
def init_db():
    ...
    if USE_POSTGRES:
        ...
        """)
    # Seed akun default secara aman.
    # PostgreSQL: ON CONFLICT mencegah error jika username sudah ada
    # (misalnya saat Streamlit melakukan rerun/restart bersamaan).
    # SQLite: INSERT OR IGNORE memberikan perilaku yang sama.
    ...
    # Seed siswa hanya jika tabel masih kosong.
    if not c.execute("SELECT 1 FROM students LIMIT 1").fetchone():
        ...
        for nis, name, class_name, parent_username, predicted_class, probability, source_idx in SEED_STUDENTS:
            if USE_POSTGRES:
                c.execute(
                    """
                    INSERT INTO students(nis, name, class_name, parent_username, created_at)
                    VALUES(?, ?, ?, ?, ?)
                ...
                c.execute(
                    """
                    INSERT OR IGNORE INTO students(nis, name, class_name, parent_username, created_at)
                    VALUES(?, ?, ?, ?, ?)
                    """,
                    (nis, name, class_name, parent_username or None, now)
                )
            ...
            c.execute(
                ...
                INSERT INTO predictions(
                    username, nis, student_name, predicted_class,
                    probability, created_at, input_json
                )
                VALUES(?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    "admin", nis, name, predicted_class, probability, now,
                    json.dumps({"seed_source_row": source_idx, "seed_data": True})
                )
            )
